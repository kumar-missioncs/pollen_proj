...
class Camera:
    ...
    def __csi_pipeline(self, sensor_id=0):
        return ('nvarguscamerasrc sensor-id=%d wbmode=0 aelock=true exposuretimerange="800000 800000" gainrange="16 16" ispdigitalgainrange="4 4" ! '
                'video/x-raw(memory:NVMM), '
                'width=(int)%d, height=(int)%d, '
                'format=(string)NV12, framerate=(fraction)%d/1 ! '
                'nvvidconv flip-method=%d left=1008 right=3024 top=760 bottom=2280 ! '
...
#                'video/x-raw, width=(int)%d, height=(int)%d, format=(string)BGRx ! '
                ...
                'video/x-raw, format=(string)BGR ! appsink' % (sensor_id,
                                                               self.hw_width, self.hw_height, self.fps, self.flip_method,
